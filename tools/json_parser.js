...
const gCommonMonsterTraits = new Array(
  ...
  "Rattle",
  "Bonesense");
function isCommonMonsterTrait(name) { for (const trait of gCommonMonsterTraits) { if (name === trait) { return true; } } return false; }
  ...
  if (array) {
    ...
    array.forEach(action => {
      ...
      if (isCommonMonsterTrait(action.name)) {
        commonTraits.push(`[${action.name}](dm/monster_info.md#monster-traits#${action.name.toLowerCase().replace(/\s/g, '-')})`);
      }
      else {
